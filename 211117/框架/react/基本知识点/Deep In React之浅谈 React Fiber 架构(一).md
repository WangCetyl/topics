...
![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg== "image-20190603163205451")
...
![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg== "image-20190625225130226")
...
-   \[UNSAFE\_\]componentWillMount（弃用）
    
-   \[UNSAFE\_\]componentWillReceiveProps（弃用）
    
-   getDerivedStateFromProps
    
-   shouldComponentUpdate
    
-   \[UNSAFE\_\]componentWillUpdate（弃用）
...
```
// 单链表树结构{   return: Fiber | null, // 指向父节点   child: Fiber | null,// 指向自己的第一个子节点   sibling: Fiber | null,// 指向自己的兄弟结构，兄弟节点的return指向同一个父节点}
```

每一个 Fiber Node 节点与 Virtual Dom 一一对应，所有 Fiber Node 连接起来形成 Fiber tree, 是个单链表树结构，如下图所示：

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/CBxTibNZG9mGMUic5C6mWniaVeMo5QVSEkeQmfzKVf8h3aE7rRriarUXzPcZORWrnthJw9lwJCA7KfrIynPxJnqLMA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
1.  觉得内容不错可以点个「在看」，让更多的人看到这篇文章
    
2.  更多技术文可以关注我的 github：https://github.com/crazylxr/blog